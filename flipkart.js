...
async function injectZeppPopup() {
  try {
    ...
    await injectAutoPopupWithData(fsn, price);
  } catch (err) {
    console.warn("Failed to find price or FSN in time:", err.message);
  }
}

async function injectAutoPopupWithData(fsn, price) {
  ...
  // Check authentication status before setting popup content
    ...
    <div style="background: #ffffff; border-radius: 0 0 12px 12px; padding: 0 0">
      ...
      <div id="priceSection" style="display: none; background: white; border-radius: 8px; margin-bottom: 16px;">
        <div style="display: flex; font-size: 16px; justify-content: space-between; padding: 12px 20px; border-bottom: 1px solid #e0e0e0;">
          <span style="color: #666;">Flipkart Price</span>
  ...
  fetch(`http://localhost:3000/api/price?id_type=${id_type}&id_value=${fsn}`)
    ...
    .then(data => {
      ...
      if (data.items && data.items.length > 0) {
        ...
        sheetPriceEl.textContent = formatIndianCurrency(finalPrice);
        if (price && savingsEl) {
...
async function loadGiftCardIfNeeded(fsn) {
  try {
    ...
    if (fsn) {
      try {
        const response = await fetch(`http://localhost:3000/api/price?id_type=fsn&id_value=${fsn}`);
        if (response.ok) {
    ...
    if (shouldShowGiftCard) {
      ...
      await showPriceComparison(fsn, isAuthenticated);
    }
...
async function showPriceComparison(fsn, isAuthenticated) {
  ...
  try {
    ...
    if (response.ok) {
      ...
      if (data.items && data.items.length > 0) {
        ...
        const flipkartPriceElement = document.getElementById('flipkartPrice');
        ...
        if (sheetPriceElement && zeppPrice > 0) {
          ...
          if (flipkartPriceElement && savingsElement) {
            const flipkartPriceText = flipkartPriceElement.textContent;
            const flipkartPrice = parseFloat(flipkartPriceText.replace(/[^\d.]/g, ''));

            if (flipkartPrice > zeppPrice) {
              const savings = flipkartPrice - zeppPrice;
              savingsElement.textContent = `You save ${formatIndianCurrency(savings)}`;
              savingsElement.style.color = '#28a745';
            } else if (zeppPrice > flipkartPrice) {
              const extra = zeppPrice - flipkartPrice;
              savingsElement.textContent = `Flipkart is ${formatIndianCurrency(extra)} cheaper`;
...
function displayGiftCardInPopup(cardData, isAuthenticated = false) {
  ...
  if (giftCardLogo) {
    ...
    giftCardLogo.textContent = cardData.title ? cardData.title.charAt(0).toUpperCase() : 'F';
  }
...
async function handleSendOtp() {
  ...
  if (!window.authManager.isValidStudentEmail(email)) {
    showInlineLoginMessage('Please enter a valid student email address', 'error');
    return;
  }
